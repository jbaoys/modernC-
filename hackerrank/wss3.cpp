...
static int pollDelay = 1000000; // usecs
/**
...
public:
    ...
    mutex mx;
    /**
     * Return a value of read from hardware.
...
Sensor1 obj1;
Sensor2 obj2, obj3;
vector<GenericSensor*> mySensors = {&obj1, &obj2, &obj3};
...
private:
    ...
    mutex m_Mtx;
    int m_State;                                /// running state of the server
                                                /// 0: stop
                                                /// 1: running
                                                /// 2: pause
public:
     ...
     * Constructor
     */
    SensorQuerySrv(vector<GenericSensor*> sensors) : m_State(0) {
        for (auto & s : sensors) {
     ...
     */
    void updatePollingState(int state)
    {
        m_State = (state >= 0 && state <3) ? state : m_State;
    };

    /**
     * Periodically running function
     * This function will be called in a seperate thread for polling all sensors
     */
     void pollSensors() {
        for (;;) {
            if (m_State == 1) {
                for (auto s : m_Sensors) {
                    /**
                     * Here, just call the sensor API to update the sensor reading.
                     * This could be block reading, or asynchronous unblock polling.
                     * I just ignore the detial implementations, please refer to
                     * the above code.
                     */
                    s.second.sensor->getValue();
                }
            } else if (m_State == 0) {
                // Stop the thread
                break;
...
int main()
{
    ...
    server.updatePollingState(1);
    thread senThread(&SensorQuerySrv::pollSensors, &server);
    ...
    if (server_sock == -1){
        cout << "SOCKET ERROR WHEN IT IS CREATED ("<<strerror(errno)<<")\n";
        exit(1);
    }
    ...
    if (rc == -1){
        cout << "FAIL TO BIND SERVER ("<<strerror(errno)<<")!!!\n";
        close(server_sock);
        exit(1);
    }
    ...
    while (1) {
        ...
        if (bytes_rec == -1){
            ...
            cmd = buf;
            // parse the request
            if (cmd == "STOP1234567890") {
                // This is the command that is used to stop the server
                server.updatePollingState(0);
                break;
            }
            sensorID = stoi(cmd);
        ...
        if (rc == -1) {
            ...
            cout << "Data sent! "<<retVal<<"\n";
        }
        close(client_sock);
    }
    close(server_sock);
    senThread.join();
    cout << "\nThe sensor server is stopped !!!\n";
    return 0;
}
